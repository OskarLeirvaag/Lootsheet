...
func CreateLootItem(ctx context.Context, databasePath string, name string, source string, quantity int, holder string, notes string) (LootItemRecord, error) {
	...
	if _, err := db.ExecContext(ctx,
		`INSERT INTO loot_items (id, name, source, status, quantity, holder, notes)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		id, name, strings.TrimSpace(source), string(service.LootStatusHeld),
...
func ListLootItems(ctx context.Context, databasePath string) ([]LootItemRecord, error) {
	...
	for rows.Next() {
		...
		item.Status = service.LootStatus(status)
...
func AppraiseLootItem(ctx context.Context, databasePath string, lootItemID string, appraisedValue int64, appraiser string, appraisedDate string, notes string) (LootAppraisalRecord, error) {
	...
	if status != service.LootStatusHeld {
		return LootAppraisalRecord{}, fmt.Errorf("loot item %q cannot be appraised: current status is %q, expected %q", lootItemID, status, service.LootStatusHeld)
	}
...
func RecognizeLootAppraisal(ctx context.Context, databasePath string, appraisalID string, date string, description string) (PostedJournalEntry, error) {
	if err := ensureInitializedDatabase(ctx, databasePath); err != nil {
	...
	if appraisalID == "" {
		return PostedJournalEntry{}, fmt.Errorf("appraisal ID is required")
	}

	date = strings.TrimSpace(date)
	if date == "" {
		return PostedJournalEntry{}, fmt.Errorf("recognition date is required")
	}
	...
	if err := db.QueryRowContext(ctx,
		...
		if err == sql.ErrNoRows {
			return PostedJournalEntry{}, fmt.Errorf("appraisal %q does not exist", appraisalID)
		}
		return PostedJournalEntry{}, fmt.Errorf("query appraisal: %w", err)
	}

	if recognizedEntryID.Valid {
		return PostedJournalEntry{}, fmt.Errorf("appraisal %q is already recognized", appraisalID)
	}
	...
	if err != nil {
		return PostedJournalEntry{}, fmt.Errorf("begin recognition transaction: %w", err)
	}
	defer tx.Rollback()
	...
	if _, err := tx.ExecContext(ctx,
		...
		return PostedJournalEntry{}, fmt.Errorf("insert recognition journal entry: %w", err)
	}
	...
	for index, line := range validated.Lines {
		if _, err := tx.ExecContext(ctx,
			...
			return PostedJournalEntry{}, fmt.Errorf("insert recognition journal line %d: %w", index+1, err)
		}
	}
	...
	if _, err := tx.ExecContext(ctx,
		...
		return PostedJournalEntry{}, fmt.Errorf("update appraisal recognized_entry_id: %w", err)
	}

	// Update loot item status to 'recognized'.
	if _, err := tx.ExecContext(ctx,
		"UPDATE loot_items SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		string(service.LootStatusRecognized), appraisal.LootItemID,
	); err != nil {
		return PostedJournalEntry{}, fmt.Errorf("update loot item status to recognized: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return PostedJournalEntry{}, fmt.Errorf("commit recognition transaction: %w", err)
	}
...
func SellLootItem(ctx context.Context, databasePath string, lootItemID string, saleAmount int64, date string, description string) (PostedJournalEntry, error) {
	if err := ensureInitializedDatabase(ctx, databasePath); err != nil {
		return PostedJournalEntry{}, err
	}
	...
	lootItemID = strings.TrimSpace(lootItemID)
	if lootItemID == "" {
		return PostedJournalEntry{}, fmt.Errorf("loot item ID is required")
	}

	if saleAmount <= 0 {
		return PostedJournalEntry{}, fmt.Errorf("sale amount must be positive")
	}

	date = strings.TrimSpace(date)
	if date == "" {
		return PostedJournalEntry{}, fmt.Errorf("sale date is required")
	}
	...
	if status != service.LootStatusRecognized {
		return PostedJournalEntry{}, fmt.Errorf("loot item %q cannot be sold: current status is %q, expected %q", lootItemID, status, service.LootStatusRecognized)
	}
	...
	if err := db.QueryRowContext(ctx,
		...
		if err == sql.ErrNoRows {
			return PostedJournalEntry{}, fmt.Errorf("loot item %q has no recognized appraisal", lootItemID)
		}
		return PostedJournalEntry{}, fmt.Errorf("query recognized appraisal: %w", err)
	}
	...
	if saleAmount < appraisedValue {
		...
		lines = append(lines, service.JournalLineInput{
	...
	tx, err := db.BeginTx(ctx, nil)
	if err != nil {
		return PostedJournalEntry{}, fmt.Errorf("begin sale transaction: %w", err)
	}
	defer tx.Rollback()
	...
	if _, err := tx.ExecContext(ctx,
		...
		return PostedJournalEntry{}, fmt.Errorf("insert sale journal entry: %w", err)
	}
	...
	for index, line := range validated.Lines {
		if _, err := tx.ExecContext(ctx,
			...
			return PostedJournalEntry{}, fmt.Errorf("insert sale journal line %d: %w", index+1, err)
		}
	}
	...
	if _, err := tx.ExecContext(ctx,
		"UPDATE loot_items SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		string(service.LootStatusSold), lootItemID,
	); err != nil {
		return PostedJournalEntry{}, fmt.Errorf("update loot item status to sold: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return PostedJournalEntry{}, fmt.Errorf("commit sale transaction: %w", err)
	}
...
func getLootItemStatus(ctx context.Context, db *sql.DB, lootItemID string) (service.LootStatus, error) {
	var status string
	if err := db.QueryRowContext(ctx,
