...
func GetAccountLedger(ctx context.Context, databasePath string, accountCode string) (AccountLedgerReport, error) {
	if err := ensureInitializedDatabase(ctx, databasePath); err != nil {
		return AccountLedgerReport{}, err
	}
	...
	accountType := service.AccountType(accountTypeStr)
	...
	// Reversed entries are excluded — their reversal entries have status='posted'.
	...
	debitNormal := accountType == service.AccountTypeAsset || accountType == service.AccountTypeExpense
