...
func GetTrialBalance(ctx context.Context, databasePath string) (TrialBalanceReport, error) {
	...
	for rows.Next() {
		...
		row.AccountType = service.AccountType(accountType)
		...
		case service.AccountTypeAsset, service.AccountTypeExpense:
			row.Balance = row.TotalDebits - row.TotalCredits
		case service.AccountTypeLiability, service.AccountTypeEquity, service.AccountTypeIncome:
