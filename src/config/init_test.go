...
func TestLoadInitAssets(t *testing.T) {
	...
	if len(assets.Migrations) != 3 {
		t.Fatalf("migration count = %d, want 3", len(assets.Migrations))
	}
	...
	if assets.SchemaVersion != "3" {
		t.Fatalf("schema version = %q, want 3", assets.SchemaVersion)
	}
